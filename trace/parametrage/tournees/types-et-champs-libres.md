...
Les types permettent de venir différencier des flux différents de tournées, qui auront chacun leur process de traitement différent.
...
* **Couleur :** Permet de définir une couleur spécifique à un type. Cette couleur sera utilisée sur l'interface web sous forme de badge dans le détail de chaque tournée. Sur l'interface nomade, la même couleur permettra d'afficher les tournées de manière visuelle et rapide, aidant les utilisateurs à reconnaître facilement le type de tournée à traiter.
* **Par défaut :** Ce type de tournée sera sélectionné par défaut lors de la création d'une demande de tournée par l'utilisateur si ce paramètre est activé.
* **Actif :** Si cette coche n'est pas active ce type de tournée ne pourra pas être utilisé.
